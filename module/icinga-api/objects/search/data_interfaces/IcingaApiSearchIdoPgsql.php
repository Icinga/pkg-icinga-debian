...
class IcingaApiSearchIdoPgsql
	...
	public $queryMap = array (
		...
		self::TARGET_HOST_STATUS_SUMMARY => 
			'select
				${FIELDS:hs.current_state AS HOST_STATE, count(DISTINCT hs.host_object_id) AS COUNT}
			from
		...
		self::TARGET_SERVICE_STATUS_SUMMARY =>
			'select
				${FIELDS:ss.current_state AS SERVICE_STATE, count(DISTINCT ss.service_object_id) AS COUNT}
			from
