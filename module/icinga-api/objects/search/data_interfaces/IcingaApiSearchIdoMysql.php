...
implements IcingaApiSearchIdoInterface {
	...
	public $statements = array (
		...
		self::TARGET_HOST_SERVICE =>
			...
			left join ${TABLE_PREFIX}objects os on os.object_id = op.object_id and op.objecttype_id = 2
			
			left join ${TABLE_PREFIX}services s on s.service_object_id = op.object_id
			left join ${TABLE_PREFIX}servicestatus ss on ss.service_object_id = op.object_id
			
			inner join ${TABLE_PREFIX}objects oh on (oh.object_id = op.object_id and op.objecttype_id = 1) OR oh.object_id = s.host_object_id
			inner join ${TABLE_PREFIX}hosts h on h.host_object_id = oh.object_id

			${if_table:i,h:left join ${TABLE_PREFIX}instances i on i.instance_id = h.instance_id}
			
			${if_table:hgm:left join ${TABLE_PREFIX}hostgroup_members hgm on hgm.host_object_id = oh.object_id}
			
			${if_table:hg,hgm:left join ${TABLE_PREFIX}hostgroups hg on hg.hostgroup_id = hgm.hostgroup_id}
			${if_table:ohg,hg,hgm:left join ${TABLE_PREFIX}objects ohg on ohg.object_id = hg.hostgroup_object_id}
			...
			left join ${TABLE_PREFIX}hoststatus hs on hs.host_object_id = oh.object_id
